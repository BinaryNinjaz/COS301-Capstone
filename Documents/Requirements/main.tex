...
\center{\textbf{Introduction}\\ 
	\flushleft{
The goal of the project is to design and develop a software system to be used by the farming community in the facilitation and management farming. The software system in are developing is Harvest that will be consisting of: an Android application, iOS application, and a website.
\newline\newline}

\textbf{Functional Requirements:}
\item[$\bullet$]On sign in Harvest shall verify credentials against the database before signing them in. 
\item[$\bullet$]Harvest will create a profile on the database for the user through a registration form.
\item[$\bullet$]Harvest must measures yield, by combining a clocking system with yield data, through the input view of the mobile interface.
\item[$\bullet$]The harvest mobile interface will make use of GPS data and based on weight and location assumptions, it must give approximate yield estimates not only for each orchard, but for each approximate location where the data was entered.
\item[$\bullet$]The Harvest website shall display data on the farm on a heat map.
\item[$\bullet$]The Web interface must accepts detailed information about produce. (e.g. cultivar, year planted)
\item[$\bullet$]The Harvest System will track the foreman through their phone/tablets GPS.
\item[$\bullet$]The Harvest System will do administrative tasks through the web interface.
\item[$\bullet$]The harvest website must give the farmer/administrator a real time map view of foremen's locations.
...
\includegraphics[width=1.2\linewidth]{persistenceFrameWork.PNG}
	\subsubsection{Determining Design Objectives}
\flushleft
		\item[$\bullet$] The design is aimed to provide accessibility, data integrity and best performance 
...
\flushleft
		The system under development is identified as an Object persistence subsystem (also known as a Database subystem), because heavily focuses on  efficient information storage and retrieval . It also hides the database details such as implementation from the sub-sytems, and  therefore uses an abstract interface to manipulate data.
		
	\subsubsection{Specifying subsystem functions, interfaces, and interaction behaviour}
\flushleft
		\item[$\bullet$]\textbf{The Application Interface: }This interface will have the functionality for login in and signing up of foremen. It will have yeild data recording functionality and worker performance capturing functionality.
