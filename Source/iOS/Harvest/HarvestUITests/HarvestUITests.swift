...
class HarvestUITests: XCTestCase {
  ...
  // IMPORTANT: Please log out of phone before running tests
  func testFarmerLogInFail() {
    ...
    app.alerts["Sign In Failure"].buttons["Okay"].tap()
  }
  
  func testFarmerLogInPass() {
    ...
    let key = app2/*@START_MENU_TOKEN@*/.keys["0"]/*[[".keyboards.keys[\"0\"]",".keys[\"0\"]"],[[[-1,1],[-1,0]]],[0]]@END_MENU_TOKEN@*/
    key.tap()
    key.tap()
    key.tap()
    key.tap()
